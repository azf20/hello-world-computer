import {
  ...
  import { EvmWalletProvider, Network, CHAIN_ID_TO_NETWORK_ID } from "@coinbase/agentkit";
  ...
  export class ViemWalletProvider extends EvmWalletProvider {
    ...
    /**
     * Signs a message. EXTENDED
     *
     * @param message - The message to sign.
     * @returns The signed message.
     */
    async signMessage(message: string | Uint8Array | { message: { raw: `0x${string}` } }): Promise<`0x${string}`> {
        if (typeof message === "string" || message instanceof Uint8Array) {
          return this.#walletClient.signMessage({ account: this.getAddress(), message: message as string });
        }
    
        return this.#walletClient.signMessage({ account: this.getAddress(), message: message.message });
      }
